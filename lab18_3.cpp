...
student * findstudent(vector<student> &allstudents, int key){ // Modified function signature
	...
	while(getline(course_file, textline)){
		if(state == 1){
			...
			if(textline == "> Students"){
				...
				allcourses.back().lecture_list.push_back(textline); // Append to lecture_list
			}            
		} else {
			if(textline == "---------------------------------------"){
				state = 1;
				...
				allcourses.back().student_list.push_back(p); // Append to student_list
			}
		}
	}
